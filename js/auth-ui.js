...
import { auth, onAuthChange, signUpWithEmail, signInWithEmail, signInWithGoogle, signOutUser, authErrorMessage } from "./auth.js";
...
function logoutIcon() {
  return `<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><path d="M16 17l5-5-5-5"/><path d="M21 12H9"/></svg>`;
}
function googleIcon() {
...
function accountSlotHTML(user) {
  ...
  <div class="account-chip">
    <span>Willkommen, ${escapeHTML(name)}</span>
    <button class="account-logout" id="account-logout" aria-label="Abmelden" type="button">${logoutIcon()}</button>
  </div>`;
}
...
function renderAccountSlot(user) {
  ...
  document.getElementById("account-logout")?.addEventListener("click", () => signOutUser());
}

// Mounted once per page load (see js/layout.js's mountLayout()) — keeps the
// header's login button / "Willkommen, X" chip in sync with the real auth
// state, including the very first render (onAuthChange fires immediately
// with the current user, same contract as Firebase's own listener).
export function mountAccountUI() {
  if (!document.getElementById("account-slot")) return;
  onAuthChange(renderAccountSlot);
}

// signUpWithEmail's updateProfile(displayName) call resolves *after*
// createUserWithEmailAndPassword already fired onAuthStateChanged once (with
// no displayName yet) -- profile field changes don't trigger the listener
// again, so without this the header would keep showing the email-derived
// fallback name until the next full reload. Called from the submit handler
// right after a successful sign-up, reading the now-updated auth.currentUser
// directly instead of waiting on a listener event that will never come.
